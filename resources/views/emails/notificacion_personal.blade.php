...
@section('content')
    ...
    <p>
        Estimado/a <strong>{{ $usuario->nombre_completo }}</strong>,
    </p>
    ...
    @if($tratamiento->citas->count())
        ...
        <ul>
            @foreach($tratamiento->citas as $cita)
                <li>
                    {{ $cita->fecha_hora->format('d/m/Y H:i') }} ({{ $cita->estado }})
                    <ul>
                        @foreach($cita->usuarios as $user)
                            @if($usuario->id == $user->id)
                                <li>
                                    {{ $user->name }} - Rol: {{ $user->pivot->rol_en_cita }}
                                </li>
                                @break
                            @endif
                        @endforeach
